...
def render_main_view():
    clear_view()
    button = Button(tk, text="List all tasks", bg="blue", fg="yellow", command=render_list_tasks_view).grid(column=0, row=0, padx=20, pady=20)
    button = Button(tk, text="Create a tasks", bg="green", fg="yellow", command=render_create_view).grid(column=1, row=0, padx=20, pady=20)
...
def render_list_tasks_view():
    ...
    dropdown = Combobox(tk, textvariable=selected_task, width = '90')
    dropdown['values'] = tasks
    dropdown.grid(row=0, column=0)
    Button(tk, text='Delete', bg='red', command=lambda: delete_task(selected_task.get())).grid(row=1, column=0)
    Button(tk, text="Cancel", bg="black", fg='yellow', command=render_main_view).grid(column=1, row=1)
...
def render_create_view():
    ...
    Label(tk, text='Enter your task priority:').grid(column=0, row=2, padx=20, pady=20)
    ...
    Button(tk, text="Create task", bg="green", fg='yellow', command=lambda: create_task(task_name.get(), task_description.get('1.0', END), task_priority.get(), task_completed.get())).grid(column=0, row=4)
    Button(tk, text="Cancel", bg="black", fg='yellow', command=render_main_view).grid(column=1, row=4)
