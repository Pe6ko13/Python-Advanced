...
def is_secondary_dia_winner(board):
    current_values = []
    n = len(board)
    for i in range(n):
...
def is_winner(board):
    if is_row_winner(board) or is_col_winner(board) or is_prime_dia_winner(board) or is_secondary_dia_winner(board):
        return True
    return False
